    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            if let Some(msg_received) = msg.downcast_ref::<MsgHello>() {
                ...
                println!(
                    "{name}: GOT on {} {msg_received:?}, STOPPING thread",
                    receiver.name
                );
                break;
            } else if let Some(msg_received) = msg.downcast_ref::<MsgReceiver>() {
                println!(
                    "{name}: GOT on {} {msg_received:?}, switching receivers",
                    receiver.name
                );
...
fn main() {
    ...
    let thread1_handle = do_it("thread1".to_string(), wait_for_receiver_rx);
    ...
    let msg = Box::new(MsgHello {
        greeting: "Hello, via tx1".to_string(),
    });
    ...
    let msg = Box::new(MsgHello {
        greeting: "Hello, via tx2".to_string(),
    });
